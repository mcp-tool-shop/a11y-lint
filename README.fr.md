...
Vérification de l'accessibilité pour les sorties de la ligne de commande, en tenant compte des besoins des utilisateurs ayant une vision réduite.
---
...
## Pourquoi ?
La plupart des outils en ligne de commande traitent la sortie des erreurs comme une fonctionnalité secondaire. Les messages tels que les erreurs "fichier introuvable" (ENOENT) ou les messages d'erreur fatals cryptiques supposent que l'utilisateur peut interpréter visuellement une sortie de terminal dense et qu'il connaît déjà ce qui s'est mal passé. Pour les utilisateurs ayant une vision réduite, des troubles cognitifs, ou pour toute personne travaillant dans un environnement stressant, ces messages constituent un obstacle.
...
### `scan` - Vérifie la présence de problèmes d'accessibilité
...
| ------ | ------ | ------ | ------------- |
| `no-color-only` | CLR001 | 1.4.1 | Ne pas transmettre d'informations uniquement par la couleur |
...
| `emoji-moderation` | SCR001 | Limitez l'utilisation des emojis (cela peut perturber les lecteurs d'écran). |
| `punctuation` | LNG003 | Les messages d'erreur doivent se terminer par une ponctuation. |
| `error-structure` | A11Y003 | Les erreurs doivent expliquer pourquoi elles se produisent et comment les corriger. |
| `no-ambiguous-pronouns` | LNG004 | Évitez de commencer une phrase par "il", "ceci", etc. |

## Format des messages d'erreur

Tous les messages d'erreur suivent la structure "Quoi/Pourquoi/Comment corriger" :
...
1. **Privilégiez les erreurs aux notes** : Utilisez des codes de sortie, pas des notes.
2. **Activez des règles spécifiques** : Pour la conformité WCAG, activez la règle `no-color-only`.
3. **Suivez les valeurs de référence** : Utilisez la sortie JSON pour détecter les régressions.
4. **Considérez les badges comme informatifs** : Ils n'impliquent pas la conformité.

## Outils complémentaires

| Outil | Description |
| ------ | ------------- |
| [a11y-ci](https://pypi.org/project/a11y-ci/) | Vérification CI pour les scores d'accessibilité avec détection de régressions par rapport aux valeurs de référence. |
| [a11y-assist](https://pypi.org/project/a11y-assist/) | Assistance déterministe pour les erreurs de l'interface en ligne de commande. |
