...
针对低视力用户的可访问性代码检查工具，用于检查命令行输出。
---
验证错误消息是否遵循可访问的模式，结构为 **[OK]/[WARN]/[ERROR] + 问题/原因/解决方案**。
## 原因
大多数命令行工具将错误输出视为次要功能。例如，ENOENT 错误或难以理解的致命错误消息，假设用户能够通过视觉方式解析终端输出，并且已经知道发生了什么问题。对于视力较差、认知障碍的用户，或者在压力下工作的人来说，这些消息会构成障碍。
...
目前，只有 `no-color-only` 规则（对应 WCAG SC 1.4.1）是映射到 WCAG 的规则。所有其他规则都是策略规则，旨在提高消息的清晰度和可读性。

### 等级 vs. CI 自动化

**重要提示：** 字母等级（A-F）是用于管理层报告的*汇总信息*。它们绝不应该作为 CI 自动化的主要依据。

对于 CI 流水线，应该基于以下内容进行自动化：
...
| 规则 | 代码 | 描述 |
| ------ | ------ | ------------- |
| `line-length` | FMT001 | 行数应限制为 120 个字符或更少 |
| `no-all-caps` | LNG002 | 避免使用全大写文本（难以阅读） |
| `plain-language` | LNG001 | 避免使用技术术语（如EOF、STDIN等）。 |
| `emoji-moderation` | SCR001 | 尽量减少表情符号的使用（表情符号会干扰屏幕阅读器）。 |
| `punctuation` | LNG003 | 错误信息应该以标点符号结尾。 |
| `error-structure` | A11Y003 | 错误信息应该解释原因以及如何修复。 |
| `no-ambiguous-pronouns` | LNG004 | 避免以“它”、“这个”等词语开头。 |

## 错误信息格式

所有错误信息都遵循“是什么/为什么/如何修复”的结构：
...
消息符合CLI错误模式（`schemas/cli.error.schema.v0.1.json`）。
...
1. **以错误为准，而非等级：** 使用退出码，而不是字母等级。
2. **启用特定规则：** 为了符合WCAG标准，启用`no-color-only`规则。
3. **跟踪基线：** 使用JSON输出来检测回归。
4. **将徽章视为信息：** 徽章不代表符合标准。
...
| 工具 | 描述 |
| ------ | ------------- |
| [a11y-ci](https://pypi.org/project/a11y-ci/) | 用于a11y-lint评分卡的CI检查，并具有基线回归检测功能。 |
| [a11y-assist](https://pypi.org/project/a11y-assist/) | 用于CLI失败的确定性辅助功能支持。 |
